package cz.humblej.squares.codec;
...
public final class GameResultCodec {
    private static final int FORMAT_VERSION = 1;

    private GameResultCodec() {
    }

    public static String encode(GameResult result) throws IOException {
    ...
    public static GameResult decode(String encoded) throws IOException {
        byte[] bytes;
        try {
            bytes = Base64.getDecoder().decode(encoded);
        } catch (IllegalArgumentException exception) {
            throw new IOException("NeplatnĂ© kĂłdovĂˇnĂ­ vĂ˝sledku hry.", exception);
        }

        try (DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes))) {
            int version = input.readInt();
            if (version != FORMAT_VERSION) {
                throw new IOException("NepodporovanĂˇ verze vĂ˝sledku hry: " + version);
            }
            ...
            if (input.available() != 0) {
                throw new IOException("VĂ˝sledek hry obsahuje neoÄŤekĂˇvanĂˇ data.");
            }
            ...
            throw new IOException("NeplatnĂ˝ strukturovanĂ˝ vĂ˝sledek hry.", exception);
        }
    }
    ...
    private static PlayerResult readPlayer(DataInputStream input, PlayerResult.Seat seat) throws IOException {
        ...
        throw new IOException("NeplatnĂˇ identita hrĂˇÄŤe ve vĂ˝sledku hry.");
    }
