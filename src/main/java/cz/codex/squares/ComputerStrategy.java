...
final class ComputerStrategy {
    ...
    static Move chooseMove(int rows, int columns, int[][] horizontalEdges, int[][] verticalEdges,
                           double skill, Random random) {
        return chooseDecision(rows, columns, horizontalEdges, verticalEdges, skill, random).move;
    }

    static Decision chooseDecision(int rows, int columns, int[][] horizontalEdges, int[][] verticalEdges,
                                   double skill, Random random) {
        ...
        private Board(int rows, int columns, int[][] horizontalEdges, int[][] verticalEdges) {
            ...
            for (int index = 0; index < moves.size(); index++) {
                ...
                selected[index] = move.horizontal
                        ? horizontalEdges[move.rowOrLine][move.columnOrLine] != SquaresPanel.NO_PLAYER
                        : verticalEdges[move.rowOrLine][move.columnOrLine] != SquaresPanel.NO_PLAYER;
