...
public final class AuthenticatedSession {
    ...
    public HttpTransport.Response putJson(String path, String json, Map<String, String> headers)
            throws AuthenticationException {
        return request("PUT", path, json, headers, false, false);
    }
    ...
    private HttpTransport.Response request(
            ...
            throws AuthenticationException {
        return request(method, path, json, Collections.<String, String>emptyMap(),
                alreadyRetried, allowNotFound);
    }

    private HttpTransport.Response request(
            String method, String path, String json, Map<String, String> headers,
            boolean alreadyRetried, boolean allowNotFound)
            throws AuthenticationException {
        ...
        try {
            ...
            response = "PUT".equals(method)
                    ? transport.putJson(uri, json, usable.getAccessToken(), headers)
        ...
        if (response.status() == 401 && !alreadyRetried) {
            usableTokens(true);
            return request(method, path, json, headers, true, allowNotFound);
        }
    ...
    private synchronized TokenSet usableTokens(boolean forceRefresh)
        ...
        if (tokens == null) {
            throw new AuthenticationException(
                    IdentityClientMessages.get(IdentityClientMessages.SESSION_MISSING),
                    null, true);
        }
    ...
    private void saveTokens(TokenSet newTokens) throws AuthenticationException {
        try {
            ...
            throw new AuthenticationException(
                    IdentityClientMessages.get(IdentityClientMessages.SESSION_STORE_FAILED),
                    exception);
        }
    }
    ...
    private AuthenticationException apiError(HttpTransport.Response response) {
        ...
        String detailSuffix = detail == null || detail.trim().isEmpty()
                ? "" : ": " + detail;
        String codeSuffix = code == null || code.trim().isEmpty()
                ? "" : " (" + code + ")";
        String message = IdentityClientMessages.get(
                IdentityClientMessages.HTTP_ERROR,
                Integer.valueOf(response.status()), detailSuffix, codeSuffix);
        return new AuthenticationException(message, null, false,
